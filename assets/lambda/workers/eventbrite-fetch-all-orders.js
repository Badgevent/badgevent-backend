...
async function _publishToSns (topicArn, message) {
  ...
  console.log(`SNS Published: topicArn=${topicArn} messageId:${response.MessageId} messageLength=${message.length}`)
}
...
export const handler = async (event, context, callback) => {
  ...
  let continuation
  do {
    const ebUrl = continuation === undefined ? `/events/${Secret.EventId}/orders/?expand=attendees` : `/events/${Secret.EventId}/orders/?expand=attendees&continuation=${continuation}`
