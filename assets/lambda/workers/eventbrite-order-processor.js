...
async function getOrCreateMember(givenName, familyName, email, badgeName) {
  ...
  if (rows.length > 0) {
    ...
    console.log(
      `CREATING MEMBER: givenName=${givenName} familyName=${familyName} email=${email} badgeName=${badgeName}`
    );
...
async function sendEventbriteRoleEmail(
  site,
  login,
  email,
  givenName,
  ...
  console.log(
    `Found event settings: eventNames=${eventNames} eventId=${eventId}`
  );
  ...
  [rows] = await dbConnection
    ...
    .catch((err) => {
      console.log(
        `Error looking up Company Name setting: ${err}\n${err.stack}`
      );
    });
  ...
  console.log(
    `sendEventbriteRoleEmail: result=${JSON.stringify(response, null, 2)}`
  );
  return response;
}

async function createExtUserRole(
  extSystemId,
  extOrderId,
  extItemId,
  memberId,
...
async function updateUserRoleExt(
  extSystemId,
  extOrderId,
  extItemId,
  role,
...
async function createOrUpdateUserRoleByExtId(
  extSystemId,
  extEventId,
  extOrderId,
  extItemId,
  ...
  console.log(
    `CREATE_OR_UPDATE_ROLE: dbUser=${dbUser} givenName=${givenName} familyName=${familyName} email=${email} role=${role} effective=${effective} expires=${expires} registeredDate=${registeredDate} badgeName=${badgeName} extOrderId=${extOrderId} extItemId=${extItemId}`
  );
  if (dbUser !== dbCurrentUser) {
    console.log(
      `Connecting to the Database: currentUser=${dbCurrentUser} => user=${dbUser}`
    );
    const dbSecrets = await getDbSecrets(dbUser);
    await dbConnect(dbSecrets);
  }
  const extUserRole = await getUserRoleByExtId(
    extSystemId,
    extOrderId,
    extItemId,
    registeredDate
  ...
  if (extUserRole) {
    ...
    if (
      extUserRole.role !== role ||
      dbEffective !== effective ||
      dbExpires !== expires
    ) {
      console.log(
        `UPDATING ROLE: oldRole=${extUserRole.role} newRole=${role} oldEffective=${dbEffective} newEffective=${effective} oldExpires=${dbExpires} newExpires=${expires}`
      );
      const result = await updateUserRoleExt(
        extSystemId,
        extOrderId,
        extItemId,
        role,
      ...
      console.log(
        `ROLE ALREADY EXISTS: id_member=${extUserRole.id_user} role=${role} effective=${effective} expires=${expires}`
      );
    }
  } else {
    ...
    if (member) {
      console.log(
        `CREATING ROLE: memberId=${member.id} extOrderId=${extOrderId} extItemId=${extItemId} role=${role} effective=${effective} expires=${expires}`
      );
      await createExtUserRole(
        extSystemId,
        extOrderId,
        extItemId,
        member.id,
  ...
  for (const record of event.Records) {
    ...
    for (const attendee of order.attendees) {
      ...
      for (const answer of attendee.answers) {
        if (
          answer.question_id === config[eventbriteEventId].question_id_badgename
        ) {
          badgeName = answer.answer;
        } else if (
          answer.question_id ===
          config[eventbriteEventId].question_id_childbadgename
        ) {
          childBadgeName = answer.answer;
        }
      }
      console.log(
        `PROCESSING orderId=${orderId} attendeeId=${attendeeId} ticketClassName=${ticketClassName} givenName=${givenName} familyName=${familyName} email=${email} badgeName=${badgeName} childBadgeName=${childBadgeName} ticketClassId=${ticketClassId} created=${createdOn}`
      );

      // TODO error handling
      const ticketClass =
        config[eventbriteEventId].ticket_classes[ticketClassId];
      if (ticketClass) {
