...
class ApiV2Service extends Construct {
  constructor(scope, id) {
    ...
    const lambdaSecurityGroupId = SSM.StringParameter.valueFromLookup(
      this,
      "badgevent-lambda-security-group-id"
    );
    let eventbriteSecretArn = SSM.StringParameter.valueFromLookup(
      this,
      "badgevent-eventbrite-api-secret"
    );
    ...
    if (eventbriteSecretArn.includes("dummy-value")) {
      eventbriteSecretArn =
        "arn:aws:service:us-east-1:123456789012:entity/dummy-value";
    }
    const eventbriteConfigStringParameter =
      SSM.StringParameter.fromStringParameterAttributes(
        this,
        "EventbriteConfig",
        { parameterName: "badgevent-eventbrite-config" }
      );
    ...
    const eventbriteApiSecret = SecretsManager.Secret.fromSecretAttributes(
      this,
      "eventbrite-api",
      { secretArn: eventbriteSecretArn }
    );

    const sesConfigurationSetName = SSM.StringParameter.valueFromLookup(
      this,
      "badgevent-ses-configuration-set-name"
    );
    const sesEmailIdentityName = SSM.StringParameter.valueFromLookup(
      this,
      "badgevent-ses-email-identity-name"
    );
    ...
    (() =>
      new SES.CfnTemplate(this, "ses-eventbrite-new-role-template", {
        template: {
          ...
          htmlPart: fs
            .readFileSync("assets/email/eventbrite-new-role.html", "utf8")
            .toString("utf-8"),
          textPart: fs
            .readFileSync("assets/email/eventbrite-new-role.txt", "utf8")
            .toString("utf-8"),
        },
      }))();
    ...
    eventbriteOrderTopic.grantPublish(new AccountRootPrincipal(accountId));
    ...
    eventbriteOrderTopic.addSubscription(
      new SNSSubscriptions.SqsSubscription(eventbriteOrderQueue, {
    ...
    const eventbriteOrderProcessorLambda = new NodeJs.NodejsFunction(
      this,
      "EventbriteOrderProcessor",
      {
        runtime: LambdaRuntime,
        entry: path.join(
          __dirname,
          "/../../assets/lambda/workers/eventbrite-order-processor.js"
        ),
        architecture: Lambda.Architecture.X86_64,
        description:
          "Creates or updates Badgevent roles based on Eventbrite orders.",
        handler: "handler",
        memorySize: 1024,
    ...
    eventbriteOrderProcessorLambda.addEventSource(
      new SqsEventSource(eventbriteOrderQueue, {
        batchSize: 10,
    ...
    eventbriteOrderProcessorLambda.addToRolePolicy(
      new PolicyStatement({
        effect: Effect.ALLOW,
        actions: [
        ...
        resources: [
          `arn:aws:secretsmanager:${region}:${accountId}:secret:dbuser-*`,
        ],
      })
    );
    eventbriteOrderProcessorLambda.addToRolePolicy(
    ...
    const eventbriteFetchAllOrdersLambda = new NodeJs.NodejsFunction(
      this,
      "EventbriteFetchAllOrders",
      {
        runtime: LambdaRuntime,
        entry: path.join(
          __dirname,
          "/../../assets/lambda/workers/eventbrite-fetch-all-orders.js"
        ),
        architecture: Lambda.Architecture.X86_64,
    ...
    // const eventbriteFetchAllOrdersScheduleRule = new Events.Rule(
    //   this,
    //   "EventbriteFetchAllOrdersScheduleRule",
    //   {
    //     schedule: Events.Schedule.rate(Duration.hours(12)),
    //   }
    // );
    // eventbriteFetchAllOrdersScheduleRule.addTarget(
    //   new Targets.LambdaFunction(eventbriteFetchAllOrdersLambda)
    // );
